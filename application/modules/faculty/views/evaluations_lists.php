<?php require_once APPPATH.'modules/common/header.php'; ?>
							...
							<table class="table rslt-tbl-row" style="font-size: 12px">
								...
								<tbody>
									...
									<tr>
										...
										<td class="text-center"><span class="view-rating" data-evl-id="<?php echo $evaluation['evaluation_id']; ?>" style="cursor:pointer;display: inline-block;background: #0a0;color: #FFF;padding: 0 8px;font-size: 11px;border-radius: 5%;" data-target="#viewFacultyRatings" data-toggle="modal"><i class="fa fa-eye"></i> View</span></td>
									</tr>
									<?php 
										$sl++;
										endforeach; 
	...
	<script type="text/javascript">
		function searchFilter(page_num) {
			...
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url(); ?>evalapi/get_evaluations_by_faculties/'+page_num,
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(document).on('click', '.view-rating', function(){
				...
				$.ajax({
					type: 'POST',
					url: baseUrl + 'evalapi/view_faculties_evaluation',
					data:{evl_id:evl_id},
					dataType:'json',
					success: function (data) {
						if(data.status == 'ok')
						{
							$('#viewFacultyRatingsContent').html(data.content);
							sqtoken_hash = data._jwar_t_kn_;
						}else
						{
							return false;
