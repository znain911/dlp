						...
						<div class="endmodule-exm-title">Write Evaluation Report</div>
							...
							<div class="form-group row">
								...
								<div class="col-lg-6">
									...
									<select class="spoecial-aff-mostak valid" name="student_id" required>
									<?php if(empty($getrtc)){
										echo '<option value="">No RTC Assign</option>';
									}else{
									foreach($getrtc as $rtc){?>
										<option value="">Select Student</option>
									<?php $getstdnts = $this->Dashboard_model->get_students($rtc->batch_id);
					    				foreach ($getstdnts as $stlist) {
					    					$totalRec = $this->Evaluation_model->count_evaluations_by_facultyidstd($stlist->student_id);
					    					...
					    					?>		
											<option value="<?php echo $stlist->student_id;?>" <?php echo $evltd;?> style="color: <?php echo $clr;?>;"><?php echo $stlist->spinfo_first_name.' '.$stlist->spinfo_middle_name.' '.$stlist->spinfo_last_name.' - '.$stlist->student_finalid;?></option>
										<?php echo 'dfsd'; } } }?>										
									</select>


									<!-- <?php 
										/*$full_name = $student_info['spinfo_first_name'].' '.$student_info['spinfo_middle_name'].' '.$student_info['spinfo_last_name'];*/
									?>
									<input type="text" name="student" class="form-control" value="<?php /*echo $full_name;*/ ?>" placeholder="Search Student" readonly />
									<input type="hidden" name="student_id" value="<?php /*echo $student_info['student_id'];*/ ?>" /> -->
								</div>
							</div>
							<input type="hidden" name="session_type" value="Student" />
