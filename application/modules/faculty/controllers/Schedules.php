...
class Schedules extends CI_Controller{
	...
	public function sdt_mrks_submit()
	{
		...
		$student_id = $this->input->post('booking_std');
		$if_already_uploaded = $this->Course_model->check_sdt_already_uploaded($booking_id, $student_id);
		if($if_already_uploaded == true)
		{
			$result = array('status' => 'error', 'content' => 'Marks already uploaded!');
	...
	public function workshop_mrks_submit()
	{
		...
		$booking_id = $this->input->post('booking');
		$student_id = $this->input->post('booking_std');
		$if_already_uploaded = $this->Course_model->check_workshop_already_uploaded($booking_id, $student_id);
		if($if_already_uploaded == true)
		{
			$result = array('status' => 'error', 'content' => 'Marks already uploaded!');
			$result[$this->sqtoken] = $this->sqtoken_hash;
