					...
					<div class="result-table">
						...
						<table class="table" style="font-size:12px;">
							...
							<tbody id="sessionContent">
								<?php require_once APPPATH.'modules/student/views/sdt/all_sessions.php' ?>
							</tbody>
						</table>
					</div>
					...
					<div class="result-table">
						...
						<table class="table" style="font-size:12px;">
							...
							<tbody id="bookingContent">
								<?php require_once APPPATH.'modules/student/views/sdt/all_bookings.php' ?>
							</tbody>
						</table>
					</div>
				</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(document).on('click', '.session-row', function(){
				...
				var get_sdt_type = $(this).attr('data-sdt-type');
				var phase_level = $(this).attr('data-phase');
				$('#ftwofScheduleLoader').show();
				$.ajax({
					type : "POST",
					url : baseUrl + "student/booking/sdt_frmsubmission",
					data : {schedule:get_schedule, get_sdt_type:get_sdt_type, phase_level:phase_level, _jwar_t_kn_:sqtoken_hash},
					dataType : "json",
