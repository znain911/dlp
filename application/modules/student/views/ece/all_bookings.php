...
$faculty_entryid = $this->Booking_model->get_ece_faculty_entryid($booking['booking_schedule_centerid']);

if($faculty_entryid !== null)
{
	$query_string = '(type='.$_GET['type'].'&to='.$_GET['to'].'&rdr=apply)';
	$href = base_url('student/evaluation?centerscheduleID='.$booking['booking_schedule_centerid'].'&faculty='.$faculty_entryid.'&RDR=ECE&RETURN='.$query_string);
}else{
	$href = 'javascript:void(0)';
}
if($booking['endmschedule_student_resource']){
	$resource_download_link = attachment_url('resources/ece/'.$booking['endmschedule_student_resource']);
}else{
