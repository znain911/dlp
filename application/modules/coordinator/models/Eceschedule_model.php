...
class Eceschedule_model extends CI_Model {
	
	public function get_schedules()
	{
		$query = $this->db->query("SELECT starter_ece_exschedule.*, starter_phases.phase_name 
								   FROM starter_ece_exschedule
								   LEFT JOIN starter_phases ON
								   starter_phases.phase_id=starter_ece_exschedule.endmschedule_phase_id
	...
	{
		$this->db->insert('starter_ece_exschedule', $data);
	}
	
	public function get_schedule_info($schedule_id)
	{
		$query = $this->db->query("SELECT * FROM starter_ece_exschedule WHERE starter_ece_exschedule.endmschedule_id='$schedule_id' LIMIT 1");
	...
	public function update_schedule($schedule_id, $data)
	{
		...
		$this->db->update('starter_ece_exschedule', $data);
	}
	...
	public function get_center_schedules($schedule_id)
	{
		$query = $this->db->query("SELECT * FROM starter_ece_centerschdl 
								   ...
								   WHERE starter_ece_centerschdl.centerschdl_parentschdl_id='$schedule_id' 
								   ORDER BY starter_ece_centerschdl.centerschdl_id DESC");
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query("SELECT endmschedule_title FROM starter_ece_exschedule WHERE starter_ece_exschedule.endmschedule_id='$schedule_id' LIMIT 1");
	...
	{
		$this->db->insert('starter_ece_centerschdl', $data);
	}
	...
	{
		$query = $this->db->query("SELECT * FROM starter_ece_centerschdl WHERE starter_ece_centerschdl.centerschdl_id='$schedule_id'");
		return $query->row_array();
	}
