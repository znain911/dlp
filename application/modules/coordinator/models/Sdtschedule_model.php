...
class Sdtschedule_model extends CI_Model {
	
	public function get_schedules($type)
	{
		$query = $this->db->query("SELECT starter_sdt_schedule.*, starter_phases.phase_name 
								   FROM starter_sdt_schedule
								   LEFT JOIN starter_phases ON
								   starter_phases.phase_id=starter_sdt_schedule.endmschedule_phase_id
								   WHERE endmschedule_sdt_type='$type'
	...
	{
		$this->db->insert('starter_sdt_schedule', $data);
	}
	
	public function get_schedule_info($schedule_id)
	{
		$query = $this->db->query("SELECT * FROM starter_sdt_schedule WHERE starter_sdt_schedule.endmschedule_id='$schedule_id' LIMIT 1");
	...
	public function update_schedule($schedule_id, $data)
	{
		...
		$this->db->update('starter_sdt_schedule', $data);
	}
	...
	public function get_center_schedules($schedule_id)
	{
		$query = $this->db->query("SELECT * FROM starter_sdt_centerschdl 
								   ...
								   WHERE starter_sdt_centerschdl.centerschdl_parentschdl_id='$schedule_id' 
								   ORDER BY starter_sdt_centerschdl.centerschdl_id DESC");
		return $query->result_array();
	}
	...
	{
		$query = $this->db->query("SELECT endmschedule_title FROM starter_sdt_schedule WHERE starter_sdt_schedule.endmschedule_id='$schedule_id' LIMIT 1");
	...
	{
		$this->db->insert('starter_sdt_centerschdl', $data);
	}
	...
	{
		$query = $this->db->query("SELECT * FROM starter_sdt_centerschdl WHERE starter_sdt_centerschdl.centerschdl_id='$schedule_id'");
		return $query->row_array();
	}
	
	public function get_total_sdt_centerschedule()
	{
		$query = $this->db->query("SELECT centerschdl_id FROM starter_sdt_centerschdl");
		$result = $query->num_rows() + 1;
		return $result;
	}
	
	public function get_sdt_schedule_title($schedule_id)
	{
		$query = $this->db->query("SELECT endmschedule_title FROM starter_sdt_schedule WHERE endmschedule_id='$schedule_id'");
		$result = $query->row_array();
		if($result['endmschedule_title'])
		{
			return $result['endmschedule_title'];
		}else{
			return null;
		}
	}
