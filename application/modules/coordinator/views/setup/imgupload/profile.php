<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Organograms</a></li>
			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/setup/organograms/edit_profile.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/setup/organograms_delete',
								...
								if(data.status == 'ok')
								{
									...
									$("#createData").validate({
										...
										submitHandler : function () {
											...
											$.ajax({
												type : "POST",
												url : baseUrl + "coordinator/setup/organograms_create",
												data : getFrmData,
												dataType : "json",
												cache: false,
												contentType: false,
				...
				$(document).on('click', '.row-action-edit', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/setup/organograms_edit",
						data:{id:id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						beforeSend: function(){
							$('#mdlBoxLoader').show();
		...
		<script type="text/javascript">
			$("#updateData").validate({
				...
				submitHandler : function () {
					...
					$.ajax({
						type : "POST",
						url : baseUrl + "coordinator/profile/organograms_update",
						data : getFrmData,
						dataType : "json",
						cache: false,
						contentType: false,
			...
			$(document).ready(function(){
				...
				$("#upPassrf").validate({
					...
					submitHandler : function () {
						...
						$.ajax({
							type : "POST",
							url : baseUrl + "coordinator/profile/organograms_change_password",
							data : getFrmData,
							dataType : "json",
							cache: false,
							contentType: false,
