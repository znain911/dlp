			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/setup/roles/contents.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/setup/roles_delete',
								...
								if(data.status == 'ok')
								{
									...
									$("#createData").validate({
										...
										submitHandler : function () {
											...
											$.ajax({
												type : "POST",
												url : baseUrl + "coordinator/setup/roles_create",
				...
				$(document).on('click', '.row-action-edit', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/setup/roles_edit",
						data:{id:id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						beforeSend: function(){
							$('#mdlBoxLoader').show();
							...
							if(data.status == 'ok')
							{
								...
								$("#updateData").validate({
									...
									submitHandler : function () {
										...
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/setup/roles_update",
				...
				$(document).on('click', '.retrive-create-again', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/setup/roles_create_item",
