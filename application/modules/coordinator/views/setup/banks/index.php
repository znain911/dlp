<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Bank Details</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/setup/banks/contents.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/setup/banks_delete',
								...
								if(data.status == 'ok')
								{
									...
									$("#createData").validate({
										...
										submitHandler : function () {
											...
											var createData = new FormData(document.getElementById('createData'));
											$.ajax({
												type : "POST",
												url : baseUrl + "coordinator/setup/banks_create",
												data : createData,
												dataType : "json",
												cache: false,
												contentType: false,
				...
				$(document).on('click', '.row-action-edit', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/setup/banks_edit",
						data:{id:id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						beforeSend: function(){
							$('#mdlBoxLoader').show();
						...
						success: function(data)
						{
							if(data.status == 'ok')
							{
								...
								$("#updateData").validate({
									...
									submitHandler : function () {
										$('#loader').show();
										var updateData = new FormData(document.getElementById('updateData'));
										// your function if, validate is success
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/setup/banks_update",
											data : updateData,
											dataType : "json",
											cache: false,
											contentType: false,
				...
				$(document).on('click', '.retrive-create-again', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/setup/banks_create_item",
