<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Manage Results</a></li>
			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/results/phase/contents.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/results/phase_delete',
								...
								if(data.status == 'ok')
								{
									...
									$("#createData").validate({
										...
										submitHandler : function () {
											...
											$.ajax({
												type : "POST",
												url : baseUrl + "coordinator/results/phase_create",
				...
				$(document).on('click', '.row-action-edit', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/results/phase_edit",
						data:{id:id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						beforeSend: function(){
							$('#mdlBoxLoader').show();
							...
							if(data.status == 'ok')
							{
								...
								$("#updateData").validate({
									...
									submitHandler : function () {
										...
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/results/phase_update",
				...
				$(document).on('click', '.retrive-create-again', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/results/phase_create_item",
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.add-more-mrksinp', function(){
					var content = '<div class="form-group">'+
										'<div class="col-md-offset-6 col-md-3"><input type="text" name="marks_label_'+row+'" class="form-control" /></div>'+
										'<div class="col-md-2 text-center relate-add-sign">'+
											'<input type="text" name="marks_'+row+'" class="form-control" />'+
				...
				$(document).on('change', '#phaseLevel', function(){
					...
					$.ajax({
						type : "POST",
						url : baseUrl + "coordinator/results/get_students",
						data:{phase_level:phase_level, _jwar_t_kn_:sqtoken_hash},
						dataType : "json",
						success : function (data) {
							if(data.status == "ok")
							{
								$('#putStudents').html(data.content);
								$('[name="_jwar_t_kn_"]').val(data._jwar_t_kn_);
								sqtoken_hash=data._jwar_t_kn_;
								return false;
							}else
