<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		<!-- end #header -->
		<?php $get_applicants = $this->Booking_model->get_workshop_applicants('Faculty'); ?>
		<!-- begin #sidebar -->
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Workshop Applicants (Faculties)</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Workshop Applicants (Faculties) : <span id="totalSdtBooking"><?php echo count($get_applicants) ?></span></h1>
			<!-- end page-header -->
								...
								<table id="data-table" class="table table-td-valign-middle m-b-0">
									...
									<tbody class="appnd-not-fnd-row">
										<?php 
											...
											$booking_status = $this->Booking_model->check_workshopbooking_status($applicant['booking_id'], $applicant['booking_user_id']);
										?>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.accept-row', function(){
					...
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/booking/as_workshop_selected',
				...
				$(document).on('click', '.decline-row', function(){
					...
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/booking/as_workshop_declined',
						data:{booking_id:booking_id, faculty_id:faculty_id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						success: function(data)
						{
