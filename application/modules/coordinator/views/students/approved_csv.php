...
<table border="1">
	<thead>
		<tr>
			...
			<th class="text-center">Badas/Non Badas</th>
	...
	<tbody>
		<?php
			...
			foreach($get_items as $item):
				...
				$name = $item['spinfo_first_name'].' '.$item['spinfo_middle_name'].' '.$item['spinfo_last_name'];
				$message ='Dear '.$name.',We are pleased to inform you that your application for admission into CCD Batch-'.$item['student_batch'].' of DLP has been approved. We would like to request you to complete the deposit of our course fee (Taka 25,000/-) via the link below :link : '.base_url('student/onboard/payment?SID='.sha1($item['student_entryid']).'&PORT='.sha1($item['student_id']).'&httpRdr=Online&status=pay').' Payment needs to be made by 20th September 2023.';
				
				if($item['approve_sms'] === '0'){
					$sms = 'Payment link pending';
				}elseif($item['approve_sms'] === '1') {
					$sms = 'All sms sent';
				}else{
					$sms = 'No sms sent';
				}
