...
<!-- <button type="button" class="btn btn-warning">Warning</button> -->
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;"><?php echo $batchdtl->batch_name;?></a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h2 class="page-header">RTC Name: <?php echo $batchdtl->batch_name;?> | Total Students <?php echo count($get_items);?></h2>

			<a href="<?php echo base_url('coordinator/students/stdntbybatch_csv/').$batchdtl->batch_id;?>" class="btn btn-info pull-right"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export</a>
			<a href="<?php echo base_url('coordinator/students/study_details_csv/').$batchdtl->batch_id;?>" class="btn btn-success pull-right"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export Study Details</a>
			<!-- end page-header -->
			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/students/enrolled/batch.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
	...
	<script type="text/javascript">
		$(document).on('click', '.row-action-study', function(){
					var loaddata = '<h1><i class="fa fa-spinner fa-spin"></i></h1>';
					$('#studydetails').html(loaddata);
					var student_id = $(this).attr('data-student');
					/*alert(student_id);*/
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/students/study_details',
						...
						success: function(data)
						{
							if(data.status == 'ok')
							{
								...
								$('#studydetails').html(data.content).slideDown();
								return false;
							}else
							{
								return false;
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.blockstudent', function(){
					var student_id = $(this).attr('data-student');
					/*alert(batchid);*/
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/students/student_blocked',
							data:{student_id:student_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							success: function(data)
							{
				...
				$(document).on('click', '.dropoutst', function(){
					var student_id = $(this).attr('data-student');
					/*alert(batchid);*/
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/students/student_dropout',
							data:{student_id:student_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							success: function(data)
							{
