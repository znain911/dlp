<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Approved Students</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Approved Students : <span id="totalEnrolledStudents"><?php echo count($get_total) ?></span></h1>

			<!--<a href="<?php echo base_url('coordinator/students/approved_csv');?>" class="btn btn-info pull-right"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export</a>-->
			...
			?>
											...
											<td class="text-center">
												<?php if($item['approve_sms'] == null):?>
													<button data-student="<?php echo $item['student_id']; ?>" class="approve-sms btn btn-default btn-xs p-l-10 p-r-10"> Approve SMS</button>
												<?php elseif($item['approve_sms'] == 0):?>
													<button data-student="<?php echo $item['student_id']; ?>" class="link-sms btn btn-default btn-xs p-l-10 p-r-10"> Send Link</button>
												<?php endif;?>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.link-sms', function(){
					
					var student_id = $(this).attr('data-student');
					//alert(student_id);
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/students/payment_link',
						data:{student_id:student_id},
						dataType: 'json',
						success: function(data)
						{
							if(data.submit == 'SMS Submitted')
							{
								alert(data.submit);
				...
				$(document).on('click', '.approve-sms', function(){
					
					var student_id = $(this).attr('data-student');
					//alert(student_id);
					$.ajax({
						type: 'POST',
						url: baseUrl+'coordinator/students/approve_sms',
						data:{student_id:student_id},
						dataType: 'json',
						success: function(data)
						{
				...
				$(document).on('click', '.row-action-status', function(){
					...
					var status_verified = '<span class="btn btn-danger btn-xs btn-rounded p-l-10 p-r-10">Verified Payment</span>';
					...
					var staus_action_verified = '<button data-status="2" data-student="'+student_id+'" class="row-action-status btn btn-info btn-xs p-l-10 p-r-10"><i class="fa fa-cog"></i> Verified Payment</button>';
					if(staus == '1')
					{
					...
					{
						$('.item-row-status-'+student_id).html(status_verified);
						$('.row-action-status-holder-'+student_id).html(staus_action_approve);
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/students/payment_verified',
							data:{student_id:student_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							success: function(data)
							{
				...
				$(document).on('click', '.exportbybatch', function(){
					var bid = $('#batch').val();
					window.location.href = baseUrl+'coordinator/students/approved_csvbybatch/'+bid;					
				});
			});
