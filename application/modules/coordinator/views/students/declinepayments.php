<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Declined Students (Payment) : <span id="totalEnrolledStudents"><?php echo count($get_total) ?></span></h1>

			<a href="#" class="btn btn-info pull-right"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export</a>
			<!-- end page-header -->
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$(document).on('click', '.row-action-status', function(){
					...
					if(staus == '1')
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/students/approve_restored',
							data:{student_id:student_id, _jwar_t_kn_:sqtoken_hash},
							dataType: 'json',
							success: function(data)
							{
			...
			function searchFilter(page_num) {
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url(); ?>studentfilter/get_paymentdeclined_student/'+page_num,
