<?php require_once APPPATH.'modules/coordinator/templates/header.php'; ?>
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<li><a href="javascript:;">Students Faqs</a></li>
			</ol>
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header">Students Faqs</h1>
			<!-- end page-header -->
			...
			<div class="row">
				...
				<div class="col-md-12 mostak-mdl-box">
					...
					<div id="moduleLessonDisplay">
						<?php require_once APPPATH.'modules/coordinator/views/faqs/students/contents.php'; ?>
					</div>
				</div>
				<!-- end col-4 -->
			</div>
			...
			$(document).ready(function(){
				...
				$(document).on('click', '.remove-row', function(){
					if(confirm('Are you sure?', true))
					{
						...
						$.ajax({
							type: 'POST',
							url: baseUrl+'coordinator/faqs/students_delete',
								...
								if(data.status == 'ok')
								{
									...
									$("#createData").validate({
										...
										submitHandler : function () {
											...
											$.ajax({
												type : "POST",
												url : baseUrl + "coordinator/faqs/students_create",
												data : getFrmData,
												dataType : "json",
												cache: false,
												contentType: false,
				...
				$(document).on('click', '.row-action-edit', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/faqs/students_edit",
						data:{id:id, _jwar_t_kn_:sqtoken_hash},
						dataType: 'json',
						beforeSend: function(){
							$('#mdlBoxLoader').show();
							...
							if(data.status == 'ok')
							{
								...
								$("#updateData").validate({
									...
									submitHandler : function () {
										...
										$.ajax({
											type : "POST",
											url : baseUrl + "coordinator/faqs/students_update",
											data : getFrmData,
											dataType : "json",
											cache: false,
											contentType: false,
				...
				$(document).on('click', '.retrive-create-again', function(){
					...
					$.ajax({
						type: "POST",
						url: baseUrl + "coordinator/faqs/students_create_item",
